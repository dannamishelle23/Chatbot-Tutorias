...
if "saludo_realizado" not in st.session_state:
    st.session_state.saludo_realizado = False  #Guardar si ya se hizo el saludo o no

#Mostrar título
st.title("Chatbot Tutorías Académicas")
st.write("Bienvenido a la plataforma de tutorías de la ESFOT!")
...
if mensaje:
    if st.session_state.estado == "normal":
        ...
        if tag_predicho == "saludo" and not st.session_state.nombre_usuario:
            ...
            #Buscar respuesta en intents.json
            respuesta = None
        ...
        #Guardar el nombre del usuario en una variable llamada "nombre"
        nombre = mensaje.strip().title()
        st.session_state.nombre_usuario = nombre
        respuesta = f"¡Gusto en conocerte, {nombre}! ¿En qué puedo ayudarte?"
